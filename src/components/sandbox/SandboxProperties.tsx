...
export function SandboxProperties({ properties = [], onChange }: Props) {
  ...
  function renderProps(p: ComponentBinding) {
    switch (p.type) {
      ...
      case "string":
        return <GoabInput name={p.name} value={p.value} width="100%" onChange={(event) => onTextChange(p.name, event.value)} />;
      case "number":
        return (
          <GoabInput step={p.step || 1} type="number" name={p.name} value={`${p.value}`}  width="100%" onChange={(event) => onNumberChange(p.name, event.value)} />
        );
      case "date":
        return (
  ...
  return (
    <div className="sandbox-container">
      {properties.filter(p => !p.hidden).map(p => (
        <GoabFormItem key={p.name} maxWidth="100%" label={p.label || ""} requirement={p.requirement as GoabFormItemRequirement || null} helpText={p.helpText}>
          {renderProps(p)}
        </GoabFormItem>
      ))}
    </div>
